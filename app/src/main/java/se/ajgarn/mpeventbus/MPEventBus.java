...
public class MPEventBus {
    ...
    public static synchronized void init(Context context) {
        if (MPEventBus.instance == null)
            MPEventBus.instance = new MPEventBus(context);
    }
    ...
    static Intent getBaseIntent() {
        ...
        intent.setAction(MPEventBus.MULTI_PROCESS_INTENT_ACTION);
    ...
    public void postToAll(CharSequence event) {
        Log.d(MPEventBus.TAG, "Sending event to all processes: " + event);
    ...
    private void postInternal(Object event) {
        Log.d(MPEventBus.TAG, "Sending event: " + event);
        EventBus.getDefault().post(event);
    }

    private void sendIntent(IntentMessageProducer eventContainer) {
        Intent broadcastIntent = MPEventBus.getBaseIntent();
        eventContainer.putExtra(MPEventBus.MULTI_PROCESS_INTENT_EXTRA, broadcastIntent);
