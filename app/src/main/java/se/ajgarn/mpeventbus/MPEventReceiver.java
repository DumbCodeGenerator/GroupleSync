...
public class MPEventReceiver extends BroadcastReceiver {
    ...
    static void register(Context context) {
        if (!MPEventReceiver.isRegisteredInManifest(context)) {
    ...
    private static Object getEvent(Intent intent, MPEventBus.EventType type) {
        switch (type) {
            case BUNDLE:
                return intent.getBundleExtra(MPEventBus.MULTI_PROCESS_INTENT_EXTRA);
            case CHAR_SEQUENCE:
                return intent.getCharSequenceExtra(MPEventBus.MULTI_PROCESS_INTENT_EXTRA);
            case PARCELABLE:
                return intent.getParcelableExtra(MPEventBus.MULTI_PROCESS_INTENT_EXTRA);
            case SERIALIZABLE:
                return intent.getSerializableExtra(MPEventBus.MULTI_PROCESS_INTENT_EXTRA);
            case STRING:
                return intent.getStringExtra(MPEventBus.MULTI_PROCESS_INTENT_EXTRA);
        }
        return null;
    }
    ...
    public void onReceive(Context context, Intent intent) {
        try {
            ...
            if (serializedType != null) {
                ...
                Object event = MPEventReceiver.getEvent(intent, type);
